...
void tarea_led( void* taskParmPtr )
{
    // ---------- CONFIGURACIONES ------------------------------
	char buffer [50];
	uint32_t vectorentrada[10]={10,20,30,40,50,60};
	uint32_t longitud=6;
    TickType_t dif;
    int ii=0;

    //muestra el vector entrada
    while(ii<longitud)
    {
    sprintf( buffer, "Elemento %d = %lu del vector de entrada \r\n", ii,vectorentrada[ii]);
    ...
    while( 1 )
    {
        vPrintString( " Al presionar el boton azul muestra el vector inicializado con sus valores a cero\r\n" );
        xSemaphoreTake( sem_btn, portMAX_DELAY );			// Esperamos tecla
        asm_zeros(vectorentrada,longitud);
...
int main( void )
{
	...
	/* USER CODE BEGIN 2 */
	vPrintString( "Main: Ejercicio_1_asm.\r\n" );
