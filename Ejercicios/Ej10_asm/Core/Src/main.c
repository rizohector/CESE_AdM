...
void task_tecla_led( void* taskParmPtr )
{
	...
	uint32_t index=0;
...

		{
          ...
          vectorIn[index]=index;
		  sprintf( buffer, "VectorIn [%lu] = %u \r\n", index,vectorIn[index]);
		  HAL_UART_Transmit(&huart3, (uint8_t*) buffer, strlen(buffer), 1000);
		  index++;

		}
		 vPrintString( "Presionar boton para realizar FUNCION _ECO \r\n" );
    ...
    while( 1 )
    {
    	...
    	if( tecla_led_state == LED)
    	{
    		        ...
    		        vPrintString( "INGRESO A LA FUNCION ASM_ECO \r\n" );
    		       ...
    		       // HAL_UART_Transmit(&huart3, (uint8_t*) buffer, strlen(buffer), 1000);
    		        ...
    		        vPrintString( "LOS VALORES DEL VECTOR_IN CON ECO SON: \r\n" );
    		        index=0;
...
int main( void )
{
  ...
  /* USER CODE BEGIN 2 */
	vPrintString( "Main: Ejercicio 10 -Language C - AdM.\r\n" );
