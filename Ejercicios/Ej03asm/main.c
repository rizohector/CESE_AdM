...
void tarea_led( void* taskParmPtr )
{
    // ---------- CONFIGURACIONES ------------------------------
	char buffer [50];
	uint16_t vectorIN[10]={10,20,30,40,50,60},vectorOUT[10]={10,20,30,40,50,60};
	uint32_t longitud=6;
	uint16_t escalar;
    TickType_t dif;
int ii=0;
    while(ii<longitud)
    {
    sprintf( buffer, "Elemento %d = %u de vectorIN\r\n", ii,vectorIN[ii]);
    ...
    while( 1 )
    {
        ...
        ii=0;
            while(ii<longitud)
            {
            sprintf( buffer, "Elemento %d = %u de vectorOUT\r\n", ii,vectorOUT[ii]);
            HAL_UART_Transmit(&huart3, (uint8_t*) buffer, strlen(buffer), 1000);
            ii++;
            }
...
int main( void )
{
	...
	/* USER CODE BEGIN 2 */
	vPrintString( "Main: Ejercicio_3_asm.\r\n" );
